...
Entry point for loading pretrained VLMs for inference; exposes functions for listing available models
(with canonical IDs, mappings to paper experiments, and short descriptions), as well as for loading
models (from disk or HF Hub).
"""
...
def load_vla(
    ...
    if os.path.isdir(model_id_or_path):
        ...
        if step_to_load is not None:
            checkpoint_pt = run_dir / "checkpoints" / f"step-{step_to_load:06d}-checkpoint.pt"
        else:
